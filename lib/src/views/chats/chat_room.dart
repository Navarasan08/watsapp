...
class _ChatRoomState extends State<ChatRoom> with TickerProviderStateMixin {
  ...
  void initState() {
    final threads = [
      Thread(fromSelf: false, message: 'Hi'),
      Thread(fromSelf: false, message: 'Poda'),
      Thread(fromSelf: true, message: 'Panni'),
      Thread(fromSelf: false, message: 'Go Away'),
      Thread(fromSelf: true, message: 'Where?'),
    ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: buildChatRoomAppBar(widget.chats.avatarUrl, widget.chats.name),
      body: Center(
        child: Column(
          children: <Widget>[
            Expanded(
